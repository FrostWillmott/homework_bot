...
def check_tokens():
    ...
    if not PRACTICUM_TOKEN:
        logging.critical('Отсутствие обязательных переменных окружения во время запуска бота: PRACTICUM_TOKEN')
        raise ValueError('Не указан токен Яндекс.Практикум')
    if not TELEGRAM_TOKEN:
        logging.critical('Отсутствие обязательных переменных окружения во время запуска бота: TELEGRAM_TOKEN')
...
def get_api_answer(timestamp):
    ...
    try:
        ...
        if homework_statuses.status_code != 200:
            ...
            raise AssertionError(f'Эндпоинт {ENDPOINT} недоступен. Код ответа: {homework_statuses.status_code}')
